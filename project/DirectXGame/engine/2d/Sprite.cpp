...
namespace MyEngine {
	...
	{
		// 頂点リソースの作成
		vertexResource_ = ResourceManager::CreateBufferResource(dxCommon_->GetDevice().Get(), sizeof(VertexData) * SpriteDefaults::kVertexCount);

		// インデックスリソースの作成
		indexResource_ = ResourceManager::CreateBufferResource(dxCommon_->GetDevice().Get(), sizeof(uint32_t) * SpriteDefaults::kIndexCount);
	...
	{
		// マテリアルリソースの作成
		materialResource_ = ResourceManager::CreateBufferResource(dxCommon_->GetDevice().Get(), sizeof(Material));
	...
	{
		// WVPリソースの作成
		wvpResource_ = ResourceManager::CreateBufferResource(dxCommon_->GetDevice().Get(), sizeof(TransformationMatrix));
