...
namespace MyEngine {
	...
	{
		cameraResource_ = ResourceManager::CreateBufferResource(
	...
	{
		pointLightResource_ = ResourceManager::CreateBufferResource(
			Object3dCommon::GetInstance()->GetDxCommon()->GetDevice().Get(),
			sizeof(PointLight));
	...
	{
		spotLightResource_ = ResourceManager::CreateBufferResource(
			Object3dCommon::GetInstance()->GetDxCommon()->GetDevice().Get(),
			sizeof(SpotLight));
	...
	void Object3d::DrawImGui()
	{
		...
		// 一時変数を使って値を取得・編集・設定
		Vector3 position = worldTransform.GetTranslate();
		Vector3 rotation = worldTransform.GetRotate();
		Vector3 scale = worldTransform.GetScale();

		if (ImGui::DragFloat3("position", &position.x, 0.01f)) {
			worldTransform.SetTranslate(position);
		}
		if (ImGui::DragFloat3("rotation", &rotation.x, 0.01f)) {
			worldTransform.SetRotate(rotation);
		}
