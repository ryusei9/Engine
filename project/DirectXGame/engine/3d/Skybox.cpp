...
namespace MyEngine {
	...
	void Skybox::Initialize(const std::string& texturePath)
	{
		...
		worldTransform_.SetScale(Vector3(kDefaultScale, kDefaultScale, kDefaultScale));
		worldTransform_.SetRotate(Vector3(kDefaultRotation, kDefaultRotation, kDefaultRotation));
		worldTransform_.SetTranslate(Vector3(0.0f, 0.0f, kDefaultTranslateZ));
	}
	...
	void Skybox::CreateMaterialResource()
	{
		...
		materialResource_ = ResourceManager::CreateBufferResource(DirectXCommon::GetInstance()->GetDevice().Get(), sizeof(Material));
	...
	void Skybox::CreateIndexBuffer()
	{
		...
		indexResource_ = ResourceManager::CreateBufferResource(DirectXCommon::GetInstance()->GetDevice().Get(), sizeof(uint32_t) * kNumIndices);
	...
	void Skybox::DrawImGui()
	{
		...
		// 一時変数を使って値を取得・編集・設定
		Vector3 position = worldTransform_.GetTranslate();
		Vector3 rotation = worldTransform_.GetRotate();
		...
		ImGui::SliderFloat3("Scale", &scale.x, 0.1f, 50.0f);
		ImGui::End();
#endif
	}
